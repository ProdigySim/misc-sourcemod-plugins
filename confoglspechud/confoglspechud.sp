...
new		bool:	g_bScoreAvailable;

#define IsScoreNativeAvailable() (g_bScoreAvailable)
...
{
	new temphp = RoundToCeil(GetEntPropFloat(client, Prop_Send, "m_healthBuffer") \
		- ((GetGameTime() - GetEntPropFloat(client, Prop_Send, "m_healthBufferTime")) \
		* GetConVarFloat(FindConVar("pain_pills_decay_rate")))) - 1;
	return temphp > 0 ? temphp : 0;
}

native ScoreMod_GetCurrentHealthBonus();
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("IsClientUsingCSpecHUD", _native_ClientUsingSpecHud);
	CreateNative("SetClientCSpecHudStatus", _native_SetClientStatus);
	RegPluginLibrary("confogl_spechud");
	return APLRes_Success;
}

public OnAllPluginsLoaded()
{
	g_bScoreAvailable = LibraryExists("l4d2_scoremod");
}

public OnLibraryAdded(const String:name[])
{
	if(StrEqual(name, "l4d2_scoremod"))
	{
		g_bScoreAvailable=true;
	}
}
...
public OnLibraryRemoved(const String:name[])
{
	if(StrEqual(name, "l4d2_scoremod"))
	{
		g_bScoreAvailable=false;
	}
}

public _native_ClientUsingSpecHud(Handle:plugin, numParams) {
	new client = GetNativeCell(1);
	return g_bSH_SpecHUD_ShowPanel[client];
}

public _native_SetClientStatus(Handle:plugin, numParams) {
	new client = GetNativeCell(1);
	new bool:status = bool:GetNativeCell(2);
	g_bSH_SpecHUD_ShowPanel[client] = status;
}
...
SH_SpecHUD_Update() {
	for(new client = 1;client < MaxClients+1;client++) {
		...
		if(g_bSH_SpecHUD_ShowHint[client]) {
			...
			PrintToChat(client,"[Confogl] Type !spechud into chat to toggle the HUD.");
		}
	}
}
...
SH_SpecHUD_Draw() {
	...
	if(IsScoreNativeAvailable())
	{
		// Health Bonus
		Format(sTempString, sizeof(sTempString), "Health Bonus: %d", ScoreMod_GetCurrentHealthBonus());
